...
<script>
    $( ".ajax_department" ).select2({
        ajax: { 
            url: '{{route("ajax_department")}}',
            type: "post",
            dataType: 'json',
            delay: 250,
            data: function (params) {
    ...
    function check(){
        ...
        document.getElementById('department_id_check').value = '';
        document.getElementById('year_post_check').value = '';

        $.ajax({
            async: false,
            type: 'GET', //THIS NEEDS TO BE GET
            url: '{{url("/ajax_chart")}}'+'/'+company_id+'/'+department_id+'/'+year_post,
        ...
        setTimeout(() => {
            ...
            OrgChart.templates.mery.field_3 = '<text class="field_3" style="font-size: 14px;" fill="#ffffff" x="180" y="70" text-anchor="middle">{val}</text>';
